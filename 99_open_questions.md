...
## Q155: Langfuse SDK/server major-version coupling (unpinned `>=2.0` resolved to v4)
**Raised during:** Session 16 (SPEC-001 Langfuse env wiring + local-trace verification)
**Question/finding:** `pyproject.toml` pinned `langfuse>=2.0`, which at install time resolved to the latest SDK (**v4.6.1**). The self-hosted server is **Langfuse v2** (ADR-003's lean Postgres-only choice). The v4 SDK speaks **OTEL ingestion** (`/api/public/otel/...`), which the v2 server doesn't expose — so traces silently failed: `Failed to export span batch code: 404`, and `auth_check()` errored parsing the v2 server's older `/api/public/projects` schema (missing `organization`/`metadata`). The read API worked (keys valid); only ingestion was broken. Net effect: local `@observe` calls emitted **zero** traces despite correct config.
**Recommended path / resolution:** Cap the SDK to the server's major version — `langfuse>=2.0,<3` — and use the v2 decorator API (`from langfuse.decorators import observe`) across the 19 `@observe` sites. After re-pinning (resolved **2.60.10**), a live `@observe` LLM call produced a trace (read-API count **0 → 1**). General lesson: **any ADR that pins a specific server major version must cap the corresponding client/SDK to that major** (`>=X,<X+1`); an open lower-bound silently tracks the SDK's latest major and breaks against an older server. `core/events/log.py` already guards both APIs (v3 `get_client` → v2 `langfuse_context` fallback), so it needs no change; if the server later upgrades to v3+, the SDK pin must follow.
**Status:** Resolved (Session 16, 2026-05-21).
